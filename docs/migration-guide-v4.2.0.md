...
```bash
npm install -g i18ntk@4.2.0
# or, for a project-local install:
npm install --save-dev i18ntk@4.2.0
```
...
If older runs showed very high missing-key counts when `sourceDir` and `i18nDir` both pointed at `./locales`, rerun usage analysis on `4.2.0`:
