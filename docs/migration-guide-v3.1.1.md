# Migration Guide: i18ntk v3.1.1

## What's New in v3.1.1

v3.1.1 hardens Auto Translate, makes placeholder-bearing strings usable by default, and reduces noisy validation warnings for branded or domain-specific copy.
It also improves sizing analysis output for projects that keep multiple JSON files in each language folder.

## Upgrade

```bash
npm install -g i18ntk@3.1.1
npm install --save-dev i18ntk@3.1.1
```
...
- Project-specific brand and product terms should be configured with `allowedEnglishTerms`; v3.1.1 no longer relies on package-level product keywords.
