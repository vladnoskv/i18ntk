...
class LocaleOptimizer {
  ...
  listLocales(options = {}) {
    ...
    console.log('\nðŸ“Š UI Locale Package Analysis');
    ...
    if (available.length === 0) {
      console.log('   âŒ No locale files found');
      return;
    }
    ...
    console.log(`\nðŸ“¦ Total package size: ${(totalSize / 1024).toFixed(1)}KB`);
    console.log(`ðŸ’¡ Potential savings: ${(totalSize/1024 - this.getLocaleSize('en')).toFixed(1)}KB (English only)`);
  ...
  keepLocales(localesToKeep) {
    ...
    console.log('ðŸŽ¯ Optimizing package size...');
   ...
   */
  restoreLocales() {
    console.log('ðŸ”„ Restoring locales from backup...');
    
    if (!SecurityUtils.safeExistsSync(this.backupDir)) {
      console.log('   âŒ No backup directory found');
      return;
    }
    ...
    if (backupFiles.length === 0) {
      console.log('   âŒ No backup files found');
      return;
    }
   ...
   */
  dryRun() {
    console.log('ðŸ” LOCALE OPTIMIZER - DRY RUN MODE');
  ...
  async interactiveSelect() {
    ...
    console.log('ðŸ“Š Available locales and sizes:');
    available.forEach(locale => {
      const size = this.getLocaleSize(locale);
    ...
    console.log('\nðŸ’¡ Type locale codes separated by commas (e.g., en,es,de)');
    ...
    if (answer.toLowerCase() === 'cancel') {
      console.log('âŒ Operation cancelled');
      return false;
    }
    ...
    console.log(`\nðŸŽ¯ Selected: ${unique.join(', ').toUpperCase()}`);
    this.showImpact(unique);
    
    const confirm = await cliHelper.prompt('\nProceed? (y/N): ');
    if (confirm.toLowerCase() === 'y' || confirm.toLowerCase() === 'yes') {
      this.keepLocales(unique.join(','));
      console.log('\nðŸŽ‰ Package optimized successfully!');
      console.log('ðŸ’¡ Use --restore to bring back removed locales');
    } else {
      console.log('âŒ Operation cancelled');
    }
    return true;
...
async function main() {
  ...
  if (!options.interactive && !options.list && !options.keep && !options.restore && !options.dryRun && !args.includes('--init')) {
    console.log('ðŸŒ i18ntk Locale Optimizer v2.0');
